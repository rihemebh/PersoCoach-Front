...
class DayForm extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:8080/api/program/" + this.props.id)
      .then(
        function (response) {
          console.log(response);
          if (response.data.dailyprogram != null)
            this.setState({
              nb: this.state.nb + response.data.dailyprogram.length,
              length: response.data.dailyprogram.length,
            });
  ...
  handleValidSubmit(event, values) {
    ...
    axios({
      ...
      function (response) {
        const notify = () =>
          toast.success("Your day program was successfully added !");
        notify();
    ...
    /* axios.post(, f).then(function (response) {
      ...
      <>
          ...
          {this.state.nb != 1 ? (
            <Col xs="1">
              <FontAwesome
                ...
                name="clear"
                onClick={this.clearDay.bind(this)}
              >
                {" "}
              </FontAwesome>
            </Col>
        ...
        >
           <h6 className="text-white"  >
               ...
               <AvField  name="breakfast" className="" type="textarea" label="" requi   style={{marginTop: "3px", marginRight: "20px"}} />
