...
?      this.setState({
  ...
  other : false
   ...
   this.setState({idProg : response.data},()=>console.log(this.state.idProg));
  ...
  componentDidMount(){
  if(this.state.idProg == 0){
    this.createProg(this.s)
  }
...
handleValidSubmit(event,values){
 ...
 axios.put("http://localhost:8080/api/program/"+this.state.id+"/day", dayProgram).then(function (response) {
    const notify = () => toast.success("Your day program was successfully added !");
    notify();
    var button = document.getElementById(this.state.id);
    button.disabled = true
