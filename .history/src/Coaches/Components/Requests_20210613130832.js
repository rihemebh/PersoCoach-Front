...
axios.put("http://localhost:3000/api/program/add", detailedProgram).then(function (response) {
  ...
  this.setState({progId : response.data},()=>console.log(this.state.progId));
...
: axios.put("http://localhost:8080/api/program/"+this.state.progId+"/day", dayProgram).then(function (response) {
  const notify = () => toast.success("Your day program was successfully added !");
  notify();
  var button = document.getElementById(this.state.id);
            ...
            <>
              ...
              <td>
                <Link
                  ...
                  to={{ pathname: "/program", request: request }}
                >
