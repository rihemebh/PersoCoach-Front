...
export default class ReviewModal extends Component {
  ...
  render() {
    ...
    this.props.reviews.length===0? console.log("no reviews"): this.props.reviews.map((value)=>{
      coach= value.coach;
      console.log(coach);
    })
    return (
      <div  onClick={this.toggleModal}>
        ...
        <Row id="modals">
          <Col>
           ...
           <span> {this.props.reviews.length} {this.props.reviews.length<2? "Review" : "Reviews" } {"for "} <h6>{coach.name}</h6></span>  
             
       
                </h5>
              </div>
              <div className="modal-body text-center" style={{ maxHeight: 'calc(100vh - 210px)',
                                overflowY: "auto"}}>
