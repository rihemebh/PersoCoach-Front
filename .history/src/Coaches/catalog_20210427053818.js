...
onClickButton= async ()=>{
  ...
  this.findCoaches(this.state.currentPage,1,)
}
 ...
 i==1?
  ...
  async findCoaches(curr,d) {
    ...
    const response = await fetch(
      "catalog/coaches?page=" +current+ "&size=" + this.state.CoachesPerPage+"&direction="+direction+"&rate="+
      this.state.rate
    );
