...
export default class Catalog extends Component {
 ...
 i==1?
  ...
  getRate= (r)=>{
    ...
    this.setState({
      rate : r,
    }, this.findCoachesByRate(this.state.rate))
  ...
  async findCoachesByRate() {
    ...
    const response = await fetch(
      "catalog/coaches?page=0&rate="+this.state.rate
    );
    const body = await response.json();
    console.log("body: ")
    console.log(body.content);
    this.setState({ coaches: body.content},console.log(this.state.coaches));
  }
